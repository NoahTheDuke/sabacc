...
let tests () =
  [
    Testo.create "roll_imposter" (fun () ->
        ...
        ());
    Testo.create "calculate_hand" (fun () ->
               ...
               |> Range.iter ~f:(fun i2 ->
                      if i1 != i2 then (
                        ...
                        check a_hand_value ~pos:__POS__
                          ...
                          (Hand.Other (abs (i1 - i2)))
                          (Game.calculate_hand hand)))));
    Testo.create "compare_hands" (fun () ->
        ...
        let player3 =
          Player.create "lavender" ~hand:(Hand.of_pair (Value 3) (Value 1))
        in
        let player4 =
        ...
        let sorted =
          ...
          |> List.stable_sort (fun (p1 : Player.t) (p2 : Player.t) ->
                 Game.compare_hands (Option.get p1.hand).value
                   (Option.get p2.hand).value)
        in
        ...
        ());
    Testo.create "calculate_round" (fun () ->
        let player1 = Player.create "noah" ~hand:(Hand.of_pair (Value 1) (Value 1)) in
